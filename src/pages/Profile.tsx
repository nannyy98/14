...
import {
  ...
  Package, Copy, Check, Pencil, X, Phone, MapPin, Clock, Sun, Moon, Monitor, Share2, MessageCircle, Sparkles,
} from 'lucide-react';
...
export const Profile = () => {
  ...
  const { getUserId, registeredName, registeredPhone, theme, setTheme } = useAppStore();
  ...
  const handleCopyReferral = () => {
    ...
    const shareText = `${language === 'ru' ? 'Присоединяйся к StyleTech Shop!' : 'StyleTech Shop ga qo\'shiling!'}\n${language === 'ru' ? 'Используй мой код:' : 'Mening kodimni ishlating:'} ${code}`;
  ...
  const handleShareReferral = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'StyleTech Shop', text: shareText });
      } catch { /* cancelled */ }
    } else {
      ...
      <div className="container mx-auto px-4 py-4 space-y-4 pb-24">
        <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-sm">
          <div className="bg-gradient-to-r from-surface-900 to-surface-800 px-5 pt-6 pb-12 rounded-t-2xl">
            <div className="flex items-start justify-between">
              <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center overflow-hidden">
                {user?.id ? (
                  <img 
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(profileData.name || user?.first_name || 'U')}&background=C9A224D&color=fff&size=64&bold=true`}
                    alt=""
          ...
          <div className="-mt-6 mx-4 bg-white dark:bg-surface-700 rounded-2xl shadow-md px-4 py-3 grid grid-cols-3 text-center divide-x divide-gray-100 dark:divide-gray-600 relative z-10">
            ...
            <div>
              <p className="text-lg font-bold text-surface-900 dark:text-white">{referrals.length}</p>
              <p className="text-xs text-surface-500 dark:text-surface-400">{language === 'ru' ? 'Рефералов' : 'Referal'}</p>
            </div>
          </div>
        ...
        {referrals.length > 0 && (
          <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-sm px-4 py-4">
            <p className="text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wide mb-3">
              {t('my_referral')}
            </p>
            <div className="flex items-center gap-2 bg-surface-50 dark:bg-surface-700 rounded-xl px-3 py-2.5">
              <code className="flex-1 text-xs text-surface-800 dark:text-gray-200 font-mono break-all">
                {referrals[0].referral_code}
              </code>
              <button
                onClick={handleCopyReferral}
                className="flex-shrink-0 p-1.5 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-600 transition text-surface-600 dark:text-surface-400"
              >
                        ...
                        {history.length > 0 && (
                          <div className="mt-2">
                            <button
                              ...
                              className="flex items-center gap-1.5 text-xs font-semibold text-surface-900 dark:text-surface-600 hover:text-surface-800 dark:hover:text-surface-400 transition mb-2"
                            >
                              <Clock className="w-3.5 h-3.5" />
                              {language === 'ru' ? 'Хронология' : 'Tarix'} ({history.length})
        ...
        <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="px-4 py-3">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-sm font-medium text-surface-900 dark:text-white">
                  {language === 'ru' ? 'Тема оформления' : 'Tema'}
                </span>
              </div>
              <div className="flex gap-2">
                {[
                  { value: 'light' as const, icon: Sun, label: language === 'ru' ? 'Светлая' : "Yorug'" },
                  { value: 'dark' as const, icon: Moon, label: language === 'ru' ? 'Тёмная' : "Qorong'u" },
                  { value: 'system' as const, icon: Monitor, label: language === 'ru' ? 'Системная' : 'Tizim' },
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-surface-100 dark:bg-surface-700 flex items-center justify-center">
                <MessageCircle className="w-4 h-4 text-surface-600 dark:text-surface-400" />
              </div>
              <span className="text-sm font-medium text-surface-900 dark:text-white">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-surface-100 dark:bg-surface-700 flex items-center justify-center">
                <Sparkles className="w-4 h-4 text-surface-600 dark:text-surface-400" />
              </div>
              <span className="text-sm font-medium text-surface-900 dark:text-white">
                {language === 'ru' ? 'Советы по стилю' : 'Uslub maslahatlari'}
              </span>
            </div>
            <ChevronRight className="w-4 h-4 text-surface-400" />
          </button>
